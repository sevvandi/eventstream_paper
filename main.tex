...
\DeclareMathOperator*{\argmax}{arg\,max}
	...
	\maketitle
	\abstract{This paper investigates early event classification in spatio-temporal data streams, when the class boundary of growing events change with the event-age. We propose a framework for early classification of such events taking into account the changing class boundary. Our framework incorporates an event extraction algorithm as well as two early event classification algorithms, which use a series of logistic regression classifiers with penalty terms and state space models.  We use this framework on synthetic and real-world applications and demonstrate its applicability and usefulness. The algorithms are available in the R package {\it eventstream}, and other code in the supplementary material.   }
	...
	\subsection{Event extraction}
	 ...
	 \begin{algorithm}[!ht]
	 	...
	 	\Input{$A$ - a 2 or 3-dimensional array, parameters $\alpha$, $\epsilon$ and {\it minPts}. }
	 	\Output{events}
