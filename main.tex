...
{\bf Scripts:} There are three files containing the R code used Section~\ref{sec:Experiments} and one other file.  The file \url{Supp_Mat_1.R}, \url{Supp_Mat_2.R} and \url{Supp_Mat_3.R} contain the code applicable for synthetic data in Section~\ref{subsec:Synthetic}, fibre optic data in Section~\ref{sec:FibreOpticExperiment} and $\text{NO}_2$ data in Section~\ref{sec:NO2Exp}. The file \url{Supp_Mat_4.R} contains the code used to produce some other graphs and images in the paper. \\
{\bf Other R-packages:} We have used the following R-packages in this paper: \textit{dma} \cite{dma}, \, \textit{abind} \cite{abind}, \, \textit{AtmRay} \cite{atmray}, \, \textit{pROC} \cite{proc}, \,  \textit{reshape2} \cite{reshape2}, \,  \textit{ggplot2} \cite{ggplot2}, \, \textit{raster} \cite{raster} and \textit{maps} \cite{maps}.   
